...
#This should go to utilities I guess

STECF_landings_BtS <- dplyr::filter(STECF_landings, grepl("27.3.B|27.3.C.22|27.3.D.24|27.3.D.25|27.3.D.28.2|
                                          27.3.D.29|27.3.D.26|27.3.D.27|27.3.D.23|27.3.D.28.1|27.3.D.30|27.3.D.31|27.3.D.32", Sub.region))
...
plot_stecf <- function(df, type, variable = NULL, cap_year, cap_month, line_count, stecf_report, return_data = FALSE) {
        ...
        my_caption = sprintf("STECF EWG 23-10. Accessed %s/%s.",
                             "October",
                             "2024")
        ...
        #Plot
plot_stecf(STECF_effort_BtS,type = "effort", variable= "COUNTRY", "2024","October", 9, "23-10", return_data = FALSE)
...
ggplot2::ggsave(paste0(cap_year, "_", ecoreg,"_FO_STECF_effortCountry.png"), path = "report/", width = 178, height = 130, units = "mm", dpi = 300)
        #data
dat <- plot_stecf(effort_BtS,type = "effort", variable= "COUNTRY", "2019","August", 9, "15-23", return_data = TRUE)
write.taf(dat, file= paste0(cap_year, "_", ecoreg,"_FO_STECF_effortCountry.csv"), dir = "report")
...
# }
        ...
        #Plot
plot_stecf(effort_BtS,type = "effort", variable= "GEAR", "2019","August", 9, "15-23")
ggplot2::ggsave(paste0(cap_year, "_", ecoreg,"_FO_STECF_effortGear.png"), path = "report/", width = 178, height = 130, units = "mm", dpi = 300)

        #data
dat<-plot_stecf(effort_BtS,type = "effort", variable= "GEAR", "2019","August", 9, "15-23", return_data = TRUE)
...
plot_stecf <- function(df, type, variable = NULL, cap_year, cap_month, line_count, stecf_report, return_data = FALSE) {
        ...
        if(type == "landings"){
                dat <- dplyr::rename(df, "type_var" ="gear_class",
                                     "VALUE" = "tot_live_weight_landed..tonnes.")
                Label <- "Landings (thousand tonnes)"
        }
        ...
        pl <- ggplot2::ggplot(dplyr::ungroup(dat), ggplot2::aes(x = Year, y = typeTotal)) +
                ggplot2::scale_fill_manual(values = myColors) +
                ggplot2::scale_color_manual(values = myColors) +
                ggplot2::scale_x_continuous(breaks = seq(min(dat$Year, na.rm = TRUE)+1,
                                                         max(dat$Year, na.rm = TRUE), by = 2)) +
                ggplot2::geom_segment(ggplot2::aes(x = -Inf, xend = 2022.5, y = -Inf, yend = -Inf), color = "grey50")+
                ggplot2::geom_segment(ggplot2::aes(y = -Inf, yend = Inf, x = -Inf, xend = -Inf), color = "grey50")+
                ggplot2::expand_limits(x = c(min(dat$Year, na.rm = TRUE), 2023)) + # So that we have enough room along x-axis for labels.
                cap_lab +
                ggplot2::theme_bw(base_size = 9) +
                ggplot2::theme(legend.position = 'none',
                               panel.grid.minor = ggplot2::element_blank(),
...
write.taf(dat, file= paste0(cap_year, "_", ecoregion,"_FO_STECF_landings.csv"), dir = "report")
