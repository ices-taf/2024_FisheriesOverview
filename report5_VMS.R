...
effort <-
  sf::st_read(paste0("vms_effort_", ecoregion, ".csv"),
               options = "GEOM_POSSIBLE_NAMES=wkt", crs = 4326)
effort <- dplyr::select(effort, -WKT)

# read vms swept area ratio
sar <-
  sf::st_read(paste0("vms_sar_", ecoregion, ".csv"),
               options = "GEOM_POSSIBLE_NAMES=wkt", crs = 4326)
sar <- dplyr::select(sar, -WKT)

# read shape ecoregion
shape_ecoregion <-
  sf::st_read(paste0("shape_ecoregion_", ecoregion, ".csv"),
              options = "GEOM_POSSIBLE_NAMES=WKT", crs = 4326)
shape_ecoregion <- dplyr::select(shape_ecoregion, -WKT)
...
# save plot
plot_effort_map(effort, shape_ecoregion) +
...
plot_sar_map(sar, shape_ecoregion, what = "surface") +
...
plot_sar_map(sar, shape_ecoregion, what = "subsurface")+
